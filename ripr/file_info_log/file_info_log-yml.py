...
# File path and database connection details from YAML file and environment variables
file_path = config['one']['file_path']
db_host = config['one']['db_host']
db_port = config['one']['db_port']
    ...
    try:
        ...
        try:
            with connection.cursor() as cursor:
                create_table_query = """
                    CREATE TABLE IF NOT EXISTS file_perms_log2 (
                ...
                insert_query = """
                    INSERT INTO file_perms_log2 (`current_timestamp`, owner, `group`, perms, fpath, file_modified_timestamp, hostname)
