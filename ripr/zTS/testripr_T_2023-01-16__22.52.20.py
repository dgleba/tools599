...
'''
# =================================================
import datetime, socket, re, uuid
...
query = db.select([t_ripr.columns.id, t_ripr.columns.host, t_ripr.columns.ipaddr, t_ripr.columns.touch]).where(t_ripr.columns.host == hostname)
results = conn.execute(query).fetchall()
df = pd.DataFrame(results)
df.columns = results[0].keys()
#print(query, results[0][0], df, results)
print(df)
print(df.at[0,'id'])
...
stmt = f"select id from ripr where host = '{hostname}'"
