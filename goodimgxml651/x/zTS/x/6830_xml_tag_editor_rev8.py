...
while True:
    ...
    def zip_back_up_of_xml_files():
        kick_out_list = []
        destination_folder_name = str(main_folder) + '-xml_back-up-' + str(datetime_now_format)
        destination_path = os.path.join(main_folder, destination_folder_name)
        os.mkdir(destination_path)
        print('Creating a back-up zip file of all .xml files in directory specified with a .csv summary report of folder contents with original pathways...Please wait.')
        
        csv_file_name = Path(destination_folder_name + '.csv')
        ...
        for subdirs, dirs, files in os.walk(main_folder):
            for filename in files:
                ...
                #Want a record of both .png and .xml, that means .png and .xml need xml-->csv conversion
                ...
                # zip_folder_name = str(destination_folder_name + '.zip')
                # # print(zip_folder_name)
                # with ZipFile(zip_folder_name, 'w') as myzip:
                #     if ext == extensions:
                #         FOLDER_NAME_everything_but_the_filename = os.path.dirname(current_file_path)
                #         myzip.write(current_file_path)
                # myzip.close()

                    # # print('Test3')
                    # # print(filename)
                    # # # print(subdirs)
                # if ext == extensions:#and current_file_path not in kick_out_list:
                    # for subdirectories in dirs:
                    # #     current_file_path = (subdirs + '\\' + filename)
                    # #     # print(current_file_path)
                    # #     # print(subdirectories)
                    #     subdirectory_path = (str(subdirectories) + '\\')
                    # FOLDER_NAME_everything_but_the_filename = os.path.dirname(current_file_path)
                    # print(FOLDER_NAME_everything_but_the_filename, os.path.basename(current_file_path))
                    # destination_folder_within_zip_file = destination_folder_
                    # print(destination_folder_2)
                    # #     # print(destination_path)
                    # if not os.path.exists(destination_folder_2):
                    #     os.mkdir(destination_folder_2)
                    # shutil.copy(current_file_path, destination_folder_2)
                    # zip_folder_name = destination_folder_name + '.zip'

                    # with ZipFile(zip_folder_name, 'w') as myzip:
                    #     myzip.write(current_file_path)
                    # myzip.close()
        filename_data.close()
        shutil.copy(csv_file_name, main_folder)


        shutil.make_archive(destination_folder_name, 'zip', destination_path)

        print('Almost done...')
        time.sleep(2)
        shutil.rmtree(destination_path)
        os.remove(csv_file_name)
        print('Back-up zip file has been generated and can be found at, \'' + str(destination_folder_name) + '.zip' + '\'. \n')
