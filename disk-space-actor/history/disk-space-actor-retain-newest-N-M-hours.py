...
It prioritizes removing the oldest files of specified types while retaining newest N files or files modified within last M hours.
It logs each operation.
"""
...
class DiskSpaceManager:
    ...
    def load_configuration(self) -> None:
        ...
        try:
            ...
            self.retain_min_files = int(os.getenv('RETAIN_MIN_FILES', 5000))
            self.retain_min_hours = int(os.getenv('RETAIN_MIN_HOURS', 24))
            ...
            logger.info(f"Configuration loaded successfully: target={self.freespace_target_mb}MB, "
                        ...
                        f"retain_min_files={self.retain_min_files}, retain_min_hours={self.retain_min_hours}")
    ...
    def filter_retained_files(self, files: List[Dict]) -> List[Dict]:
        """Filter out files that should be retained based on retention rules."""
        if not files:
            return []
        ...
        # Calculate cutoff time for minimum hours retention
        cutoff_time = time.time() - (self.retain_min_hours * 3600)
        
        # Determine which files to retain
        retained_files = []
        
        # First retain newest N files regardless of age
        retained_files.extend(sorted_files[:self.retain_min_files])
        
        # Then retain any files modified within the retention period
        for file in sorted_files[self.retain_min_files:]:
            if file['mtime'] > cutoff_time:
                retained_files.append(file)
        
        # Get the unique set of retained files (in case files were added by both rules)
        unique_retained = {f['path']: f for f in retained_files}.values()
        
        # Files to be considered for deletion are those not in the retained set
        all_files = {f['path']: f for f in files}
        retained_paths = {f['path'] for f in unique_retained}
        
        files_for_deletion = [
            all_files[path] for path in all_files 
            if path not in retained_paths
        ]
        
        logger.info(f"Retaining {len(unique_retained)} files (newest {self.retain_min_files} "
                   f"and files modified within last {self.retain_min_hours} hours)")
        logger.info(f"{len(files_for_deletion)} files available for deletion")
    ...
    def run(self) -> int:
        ...
        # Filter out files that should be retained
        files_for_deletion = self.filter_retained_files(files)
        
        if not files_for_deletion:
            logger.warning("No files available for deletion after applying retention rules")
            return 1
        
        # Delete files until target is reached or limit is hit
        success = self.delete_files(files_for_deletion, free_space_mb)
